  ...
  return (
    ...
    <div className="min-h-screen bg-background pb-20 lg:pb-0 lg:pl-20">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-20 flex-col items-center bg-sidebar py-5 text-sidebar-foreground lg:flex">
        <Link to="/dashboard" aria-label={translate("Dashboard")} className="grid size-10 place-items-center rounded-lg hover:bg-sidebar-accent">
        ...
        <nav className="mt-8 flex flex-1 flex-col gap-1">
          {sidebarNav.map((item) => (
            <Tooltip key={item.to}>
              ...
              <TooltipContent side="right" sideOffset={8}>{translate(item.label)}</TooltipContent>
            </Tooltip>
          ))}
        </nav>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-7 place-items-center border-t border-border bg-background/95 backdrop-blur py-3 lg:hidden">
        {nav.map((item) => (
          <Tooltip key={item.to}>
            ...
            <TooltipContent side="top" sideOffset={8}>{translate(item.label)}</TooltipContent>
          </Tooltip>
        ))}
      </nav>
    </div>
