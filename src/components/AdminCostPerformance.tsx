...
const fmtCost = (value: number | null) =>
  value === null ? ND : new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(value);
    ...
    <section className="pt-2">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Benchmark técnico e econômico somente leitura</p>
        </div>
        <div className="flex items-center gap-1">
          ...
          <Button size="icon" variant="ghost" aria-label="Atualizar benchmark" onClick={() => void query.refetch()} disabled={query.isFetching}>
      ...
      {query.isPending ? (
        <div className="flex items-center gap-2 py-8 text-sm text-muted-foreground"><Loader2 className="size-4 animate-spin" /> Carregando</div>
      ) : query.isError ? (
        <p className="py-8 text-sm text-muted-foreground">Não foi possível carregar o benchmark agora.</p>
      ) : (
        <>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-3">
            {[
              ...
              <div key={String(label)} className="rounded-md border border-border p-3">
                <p className="text-xs text-muted-foreground">Projeção · baseada no custo registrado</p>
                <p className="font-medium">{label}: {fmtCost(value as number | null)}</p>
              </div>
            ))}
          </div>

          <p className="mt-3 text-xs text-muted-foreground">
            Observado: chamadas, tokens, latência, erros e HITs cumulativos do cache. Calculado: totais e médias. Estimado: custo e projeções somente quando todos os registros possuem custo. First token, first chunk, retries, fallback e cache MISS: N/D.
          </p>
          {query.data.truncated && <p className="mt-1 text-xs text-muted-foreground">Amostra limitada aos 10.000 registros mais recentes.</p>}
            ...
            <table className="w-full min-w-[1680px] text-left text-xs">
              <thead className="sticky top-0 bg-background">
                <tr className="border-b border-border font-semibold uppercase text-muted-foreground">
                  {[
                    "Operação", "Provider", "Modelo", "Chamadas", "Input", "Output", "Total", "Latência", "First token", "First chunk", "Erros", "Retries", "Fallback", "Cache HIT", "Cache MISS", "Créditos", "Custo", "1k", "10k", "100k",
                  ].map((heading) => <th key={heading} className="py-2 pr-3">{heading}</th>)}
                </tr>
              </thead>
              <tbody>
                {query.data.comparisons.map((row) => (
                  <tr key={`${row.operation}-${row.model}`} className="border-b border-border/60 align-top">
                    ...
                    <td className="pr-3">{fmtN(row.totalTokens)}<div className="text-muted-foreground">{row.tokenCoverage}/{row.calls} medidos</div></td>
                    <td className="pr-3">{fmtMs(row.avgMs)}</td>
                    <td className="pr-3">{ND}</td><td className="pr-3">{ND}</td>
                    <td className="pr-3">{row.errors}</td>
                    <td className="pr-3">{ND}</td><td className="pr-3">{ND}</td>
                    <td className="pr-3">{row.cacheHits}<div className="text-muted-foreground">cumulativo</div></td>
                    <td className="pr-3">{ND}</td><td className="pr-3">{ND}</td>
