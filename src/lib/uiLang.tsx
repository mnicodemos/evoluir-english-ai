...
export function UiLangToggle({ className }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={
        "rounded-lg border border-border px-2 py-1 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground transition-colors hover:bg-accent hover:text-foreground " +
        (className ?? "")
      }
    >
      {lang === "en" ? "EN" : "PT"}
