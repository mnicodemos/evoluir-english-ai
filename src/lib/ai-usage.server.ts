...
export async function reserveAiUsage(input: {
  ...
  const { data: limit, error: limitError } = await db
    .from("ai_limits")
    .select("daily_limit, monthly_limit, premium_daily_limit, premium_monthly_limit, min_interval_seconds, max_concurrent, enabled")
    .eq("operation", input.operation)
    .single();
  if (limitError || !limit) throw new AiUsageError(503, "limits_unavailable", "AI usage controls are temporarily unavailable.");
  if (!limit.enabled) throw new AiUsageError(503, "operation_disabled", "This AI feature is temporarily unavailable.");
  ...
  if (error || !reservation) throw new AiUsageError(503, "limits_unavailable", "AI usage controls are temporarily unavailable.");
...
export async function finishAiUsage(
  ticket: UsageTicket,
  result: { success: boolean; errorCode?: string; errorMessage?: string; inputTokens?: number; outputTokens?: number },
) {
  const db = await admin();
  await db
...
export async function operationCacheTtl(operation: AiOperation): Promise<number> {
  const db = await admin();
  const { data } = await db.from("ai_limits").select("cache_ttl_seconds").eq("operation", operation).maybeSingle();
